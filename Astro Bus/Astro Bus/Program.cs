...
namespace IngameScript
{
    partial class Program : MyGridProgram
    {
        ...
        Vector3D waypoint0 = new Vector3D(12866.70, -12396.39, 58596.22);   // Approche Co Planet
        Vector3D waypoint1 = new Vector3D(12840.37, -12371.02, 58476.35);   // Co Planet
        Vector3D waypoint2 = new Vector3D(76418.03, 8110.16, 87685.90);   // Passage Retour
        Vector3D waypoint3 = new Vector3D(76376.90, 8336.94, 87705.34);   // Approche Co Station
        Vector3D waypoint4 = new Vector3D(76355.10, 8357.06, 87709.31);   // Co Station
        ...
        public bool checkList()
        {
            ...
            panel0 = GridTerminalSystem.GetBlockWithName("Astro Bus - [LCD]") as IMyTextSurface;
            if (panel0 == null)
            {
                if (err_TXT.Contains("No Astro Bus - [LCD] !!") == false)
                { err_TXT += "\nNo Astro Bus - [LCD] !!"; }
                isOK = false;
            }
            else
            ...
            if (lcds.Count != 0)
            {
                ...
                {
                    if (lcd.CustomName.Contains("Astro Bus - LCD Coin Passager"))
                    { lcd_Top.Add(lcd); }
                }
            }
            if (lcd_Top.Count == 0)
            {
                if (err_TXT.Contains("No Astro Bus - LCD Coin Passager !!") == false)
                { err_TXT += "\nNo Astro Bus - LCD Coin Passager !!"; }
                isOK = false;
            }
            else
            {
            ...
            hingeRamp = GridTerminalSystem.GetBlockWithName("Astro Bus - Hinge Ramp") as IMyMotorAdvancedStator;
            if (hingeRamp == null)
            {
                if (err_TXT.Contains("No Astro Bus - Hinge Ramp !!") == false)
                { err_TXT += "\nNo Astro Bus - Hinge Ramp !!"; }
                isOK = false;
            }
            else
            {
            ...
            rc_AstroBus = GridTerminalSystem.GetBlockWithName("Astro Bus - Remote Control") as IMyRemoteControl;
            if (rc_AstroBus == null)
            {
                if (err_TXT.Contains("No Astro Bus - Remote Control !!") == false)
                { err_TXT += "\nNo Astro Bus - Remote Control !!"; }
                isOK = false;
            }
            else
            {
            ...
            co_AstroBus = GridTerminalSystem.GetBlockWithName("Astro Bus - Connector") as IMyShipConnector;
            if (co_AstroBus == null)
            {
                if (err_TXT.Contains("No Astro Bus - Connector !!") == false)
                { err_TXT += "\nNo Astro Bus - Connector !!"; }
                isOK = false;
            }

            tb_SlowDown = GridTerminalSystem.GetBlockWithName("Astro Bus - BM - SlowDown") as IMyTimerBlock;
            if (tb_SlowDown == null)
            {
                if (err_TXT.Contains("No Astro Bus - BM - SlowDown !!") == false)
                { err_TXT += "\nNo Astro Bus - BM - SlowDown !!"; }
                isOK = false;
            }
            else
            { tb_SlowDown.ApplyAction("OnOff_On"); }

            tb_Start = GridTerminalSystem.GetBlockWithName("Astro Bus - BM - Start") as IMyTimerBlock;
            if (tb_Start == null)
            {
                if (err_TXT.Contains("No Astro Bus - BM - Start !!") == false)
                { err_TXT += "\nNo Astro Bus - BM - Start !!"; }
                isOK = false;
            }
            else
            { tb_Start.ApplyAction("OnOff_On"); }

            tb_goplanet = GridTerminalSystem.GetBlockWithName("Astro Bus - BM - Retour Station") as IMyTimerBlock;
            if (tb_goplanet == null)
            {
                if (err_TXT.Contains("No Astro Bus - BM - Retour Station !!") == false)
                { err_TXT += "\nNo Astro Bus - BM - Retour Station !!"; }
                isOK = false;
            }
            else
            { tb_goplanet.ApplyAction("OnOff_On"); }

            door = GridTerminalSystem.GetBlockWithName("Astro Bus - Door") as IMyDoor;
            if (door == null)
            {
                if (err_TXT.Contains("No Astro Bus - Door !!") == false)
                { err_TXT += "\nNo Astro Bus - Door !!"; }
                isOK = false;
            }

            sensorExt_AstroBus = GridTerminalSystem.GetBlockWithName("Astro Bus - Sensor Exterior") as IMySensorBlock;
            if (sensorExt_AstroBus == null)
            {
                if (err_TXT.Contains("No Astro Bus - Sensor Exterior !!") == false)
                { err_TXT += "\nNo Astro Bus - Sensor Exterior !!"; }
                isOK = false;
            }
            else
            {
